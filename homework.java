...
class homework {
  ...
  public void run() throws IOException {
	//Use BufferedReader rather than RandomAccessFile; it's much faster
	  ...
	  PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter("homework.out")));
	  ...
	  System.out.println("HereA");
	    ...
	    for(int ind = 0; ind < N; ind++){
	    	scores[ind] = Integer.parseInt(st.nextToken());
	    }
	    
	    System.out.println("HereB");
	  ArrayList<Integer> maximumScoreK = new ArrayList<Integer>();
	  double maximumScore = -1;
	  double[] finalScores = new double[N];
	  boolean inserted = false;
	  for(int K = 1; K <= N-2; K++){
		  if(finalScores[K]<finalScores[K+1]){
			  continue;
		  }
		  if(inserted==true&&finalScores[K]<finalScores[K-1]){
			  inserted=false;
		  }
		  System.out.println("HereC");
		  int[] newArr = parseArray(scores, K);
		  System.out.println("Here");
		  int[] newArrSorted = sortIA(newArr);
		  System.out.println("Here");
		  int lowestScore = newArrSorted[0];
		  double score = avgX0(newArrSorted);
	  ...
	  for(int K = 1; K<=N-2; K++){
		  if(finalScores[K]==maximumScore){
			  maximumScoreK.add(K);
		  }
	  }
	  System.out.println("HereE");
	  for(int ind = 0; ind < maximumScoreK.size(); ind++){
		  writer.println(maximumScoreK.get(ind));
	  }
  ...
  public int[] parseArray(int[] scores, int K){
	  int[] parsed = new int[scores.length-K];
	  for(int ind = K; ind < scores.length; ind++){
		  parsed[ind-K]=scores[ind];
	  }
	  return parsed;
  }
  
  public double avgX0(int[] arr){
	  double score=0.0;
	  for(int ind = 1; ind<arr.length;ind++){
		  score+=arr[ind];
	  }
	  score = score / (double)(arr.length-1);
	  return score;
  }
